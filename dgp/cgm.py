...
class CausalGraphicalModel:
    ...
    def __init__(self, nodes, edges, latent_edges=None, set_nodes=None):
        ...
        self.non_edges = [pair for pair in list(combinations(self.dag.nodes(), 2)) if not self.dag.has_edge(*pair)]
        self.non_path_connected =  [pair for pair in list(combinations(self.dag.nodes(), 2)) if not nx.has_path(self.dag, pair[0], pair[1])]

        # to find nodes with no parents (topmost nodes)
        self.no_parents = [node for node, degree in self.dag.in_degree() if degree == 0]
        ...
        distances = nx.all_pairs_shortest_path_length(self.graph)
        self.distances = {}
        for d in distances:
    ...
    def get_local_from_center(self, center, distance) -> set:
        """
    ...
    def arrange_topological_order(self, list_of_nodes_list):
        '''
        Take a selection of nodes from the graph and arrange them in topological
        order.
        '''

        if (isinstance(list_of_nodes_list[0], list) or isinstance(list_of_nodes_list[0], tuple)):
          list_of_sorted_nodes = []

          for nodes_list in list_of_nodes_list:
            # Get the subgraph for the given nodes
            subgraph = self.dag.subgraph(nodes_list)

            # Get the topological sort of the nodes
            sorted_nodes = list(nx.topological_sort(subgraph))
            list_of_sorted_nodes.append(sorted_nodes)

          return list_of_sorted_nodes

        else:
          subgraph = self.dag.subgraph(list_of_nodes_list)
          # Get the topological sort of the nodes
          sorted_nodes = list(nx.topological_sort(subgraph))
          return sorted_nodes
